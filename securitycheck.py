...
def input_builder(worker_index, worker_count):
    ...
    # in the first worker.
    if worker_index == 0:
        # Replace 'your_wifi_interface' with the appropriate Wi-Fi interface name
        capture = pyshark.LiveCapture(interface='uwosecure-v2', bpf_filter="tcp")
        epoch = 0
...
class Packets:
    ...
    def _calc_entropy(self, size_bytes, r):
        ...
        residual = p_new * math.log2(p_new) - p_old * math.log2(p_old)
        self.entropy = self.count_seen_packets * (self.entropy - math.log2(self.count_seen_packets / (self.count_seen_packets + r))) / (self.count_seen_packets + r) - residual
